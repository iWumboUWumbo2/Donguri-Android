...
/// An interface rather than a direct `HoshiDicts` call so the repository can be
/// unit tested without loading the native library.
interface DictionaryNativeBridge {
    ...
    fun rebuildQuery(session: Long, termPaths: Array<String>, freqPaths: Array<String>, pitchPaths: Array<String>) = Unit
...
@Singleton
class HoshiDictionaryNativeBridge @Inject constructor() : DictionaryNativeBridge {
    ...
    override fun destroyLookupObject(session: Long) = HoshiDicts.destroyLookupObject(session)
    ...
    ) = HoshiDicts.rebuildQuery(session, termPaths, freqPaths, pitchPaths, emptyArray())
    ...
    override fun lookup(
        ...
        maxResults: Int,
        scanLength: Int,
    ): List<LookupResult> = HoshiDicts.lookup(session, text, maxResults, scanLength).toList()
